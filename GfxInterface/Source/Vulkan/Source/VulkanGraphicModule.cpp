...
namespace GFXI
{
    ...
    GraphicDevice* GraphicModuleVulkan::CreateDevice()
    {
        ...
        const float QueuePriorities[] = { 0.0f, 0.0f, 0.0f };
        ...
        if (RetCreateDevice == VkResult::VK_SUCCESS)
        {
            return new GraphicDeviceVulkan(this, Info.Device, VulkanDevice
                , VulkanCommandQueue(VulkanDevice, Info.GraphicQueueFamilyIndex, Info.GraphicQueueIndex)
                , VulkanCommandQueue(VulkanDevice, Info.ComputeQueueFamilyIndex, Info.ComputeQueueIndex)
                , VulkanCommandQueue(VulkanDevice, Info.TransferQueueFamilyIndex, Info.TransferQueueIndex)
            );
        }
